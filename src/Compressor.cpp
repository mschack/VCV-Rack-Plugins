...
Compressor_Widget( Compressor *module )
{
    ...
    // bypass switch
    pmod->m_pButtonBypass = new MyLEDButton( x, y, 11, 11, 8.0, DWRGB( 180, 180, 180 ), DWRGB( 255, 0, 0 ), MyLEDButton::TYPE_SWITCH, 0, module, Compressor_Bypass );
    ...
    pmod->m_pLEDMeterThreshold = new CompressorLEDMeterWidget( true, x + 39, y + 25, 5, 3, DWRGB( 245, 10, 174 ), DWRGB( 96, 4, 68 ) );
    addChild( pmod->m_pLEDMeterThreshold );

    pmod->m_pLEDMeterComp[ 0 ] = new CompressorLEDMeterWidget( true, x + 48, y + 25, 5, 3, DWRGB( 0, 128, 255 ), DWRGB( 0, 64, 128 ) );
    addChild( pmod->m_pLEDMeterComp[ 0 ] );
    pmod->m_pLEDMeterComp[ 1 ] = new CompressorLEDMeterWidget( true, x + 55, y + 25, 5, 3, DWRGB( 0, 128, 255 ), DWRGB( 0, 64, 128 ) );
    ...
    // add param knobs
    y2 = y + 149;
    addParam(createParam<Knob_Yellow1_26>( Vec( x + 11, y + 113 ), module, Compressor::PARAM_INGAIN ) );
    addParam(createParam<Knob_Yellow1_26>( Vec( x + 62, y + 113 ), module, Compressor::PARAM_OUTGAIN ) );
    addParam(createParam<Knob_Blue2_26>( Vec( x - 5, y2 + 20 ), module, Compressor::PARAM_SIDE_CHAIN ) );
    addParam(createParam<Knob_Yellow1_26>( Vec( x + 39, y2 ), module, Compressor::PARAM_THRESHOLD ) ); y2 += 40;
    addParam(createParam<Knob_Yellow1_26>( Vec( x + 39, y2 ), module, Compressor::PARAM_RATIO ) ); y2 += 40;
    addParam(createParam<Knob_Yellow1_26>( Vec( x + 39, y2 ), module, Compressor::PARAM_ATTACK ) ); y2 += 40;
...
bool Compressor::ProcessCompState( COMP_STATE *pComp, bool bAboveThreshold )
{
    ...
    {
        pComp->count = 10 + (int)( MAX_REL_TIME * APP->engine->getSampleRate() * params[ PARAM_RELEASE ].value );
        pComp->fade = 1.0f;
    ...
    {
    case COMP_START:
        pComp->count = 10 + (int)( MAX_ATT_TIME * APP->engine->getSampleRate() * params[ PARAM_ATTACK ].value );
...
void Compressor::process(const ProcessArgs &args) 
{
    ...
    if( !m_bBypass )
    {
        ...
        {
            fside = clamp( ( inputs[ IN_SIDE_CHAIN ].getNormalVoltage( 0.0 ) / AUDIO_MAX ) * params[ PARAM_SIDE_CHAIN ].getValue(), -1.0f, 1.0f );
